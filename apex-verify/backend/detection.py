...
class DetectionPipeline:
    ...
    def segment_with_sam2(self, image: Image.Image, detections: List[DetectedObject]) -> Tuple[List[DetectedObject], float, np.ndarray | None]:
        ...
        for det in detections:
            ...
            # SAM2 may return masks at an internal resolution that differs from the
            # original image size. Resize to (img_h, img_w) before merging so that
            # np.logical_or doesn't raise a shape-broadcast error.
            ...
            if global_mask is None:
                global_mask = mask_bool.copy()
            else:
                global_mask = np.logical_or(global_mask, mask_bool)
