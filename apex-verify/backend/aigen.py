...
class AIImageDetector:
    ...
    def predict(self, image: Image.Image) -> Dict:
        ...
        # Calibrate: The base model often has high false-positive rates on real-world 
        # complex scenes (since it may be trained on smaller or specific datasets).
        # We apply a strict threshold and scaling to suppress false positives.
        if ai_prob < 0.85:
            # Heavily dampen low/medium confidence signals
            ai_prob = ai_prob * 0.2
        else:
            # Scale the [0.85, 1.0] range to [0.5, 1.0] roughly
            ai_prob = 0.5 + ((ai_prob - 0.85) / 0.15) * 0.5

        return {
            "ai_gen_score": round(ai_prob, 3),
